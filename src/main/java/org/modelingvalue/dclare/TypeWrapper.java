...
public interface TypeWrapper {
    ...
    private static List<TypeWrapper> linearization(TypeWrapper type) {
        ...
        var ls = supers.map(LINEARIZATION::get).asList();
    ...
    private static List<TypeWrapper> merge(List<TypeWrapper> result, List<List<TypeWrapper>> supers, List<TypeWrapper> immediates) {
        var combined = supers.add(immediates).filter(m -> !m.isEmpty()).asList();
        if (combined.isEmpty()) {
            return result;
        } else {
            var supersFiltered = supers.filter(m -> !m.isEmpty()).asList();
            TypeWrapper candidate = getMergeCandidate(combined).or(() -> getMergeCandidate(supersFiltered)).orElseGet(() -> combined.get(0).get(0));
            return merge(result.add(candidate), supersFiltered.map(l -> l.remove(candidate)).asList(), immediates.remove(candidate));
        }
    }
    ...
    private static Optional<TypeWrapper> getMergeCandidate(List<List<TypeWrapper>> merge) {
        for (var list : merge) {
            var head = list.get(0);
            if (merge.noneMatch(l -> l.lastIndexOf(head) > 0)) {
    ...
    class ObjectType extends IdentifiedBy<Class<?>> implements TypeWrapper {
        ...
        public List<TypeWrapper> computeSupers() {
            if (this.id() == Object.class) {
                ...
                return sups.add(this.id().getSuperclass() == null ? new ObjectType(Object.class) : new ObjectType(this.id().getSuperclass()));
            }
        }
